...
public float f {
    ...
    public static JSONObject obj { get { return new JSONObject(JSONObject.Type.OBJECT); } }     //an empty objectAAAAAA
        ...
        if (str != null)
        {
            ...
            if (str.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
#if USEFLOAT
                        number = System.Convert.ToSingle(str);
#else
					number = System.Convert.ToDouble(str);				 
#endif
                        type = Type.NUMBER;
                    }
                                ...
                                if (str[offset - 1] != '\\')
                                {
                                    if (openQuote)
                                    {
                                        if (!inProp && depth == 0 && type == Type.OBJECT)
                                            propName = str.Substring(token_tmp + 1, offset - token_tmp - 1).Replace("\\\"", "\"");
                                        openQuote = false;
                                    }
                                    else
                                    {
        ...
        if (type == Type.OBJECT)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                switch (val.type)
                {
                    ...
                    case Type.NUMBER: result.Add((string)keys[i], val.number + ""); break;
                    case Type.BOOL: result.Add((string)keys[i], val.boolean + ""); break;
